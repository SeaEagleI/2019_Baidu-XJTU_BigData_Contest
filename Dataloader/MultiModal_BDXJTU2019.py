...
class MM_BDXJTU2019(data.Dataset):
    def __init__(self, root, mode, transform = None, TRAIN_IMAGE_DIR = 'train_image_raw'):
    ...
    def __getitem__(self, index):
        ...
        Img = cv2.imread(os.path.join(self.root, self.TRAIN_IMAGE_DIR, item))
        ...
        #Img = Img - self.mean
        image_tensor = torch.from_numpy(Img).type(torch.FloatTensor)#.permute(2, 0, 1)
        ...
        anos = self.cls_to_id[item[0:3]]
        #image_tensor = tr.Normalize(torch.,image_tensor)

        # return a input data like (3, 224, 224) and an interger signifies the class
        return image_tensor, visit_tensor, anos
...
class MM_BDXJTU2019_TTA(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return a input data like (3, 224, 224) and an interger signifies the class
        return Input_O, Input_H, Input_V, visit_tensor, anos
...
class BDXJTU2019_test(data.Dataset):
    def __init__(self, root, TEST_IMAGE_DIR = 'test'):
        self.root = root
        self.TEST_IMAGE_DIR = TEST_IMAGE_DIR
        self.ids = os.listdir(os.path.join(self.root, self.TEST_IMAGE_DIR))
        self.ids.sort()
    ...
    def __getitem__(self, index):
        item = self.ids[index]
        Img = cv2.imread(os.path.join(self.root, self.TEST_IMAGE_DIR, item))
        Img = cv2.resize(Img, (100, 100))
        Img = Img[:, :, (2, 1, 0)]
        Img = self.normal(Img)
        Img = np.asarray(Img)
        ...
        return image_tensor, visit_tensor, item
...
class BDXJTU2019_test_TTA(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return Input_O, Input_H, Input_V, visit_tensor, item
